...
if (backToUsersBtn) {
  backToUsersBtn.addEventListener("click", () => {
    if (window.innerWidth <= 768) {
      history.back(); // Triggers popstate to gracefully return to list
    }
  });
}
...
window.addEventListener("popstate", (e) => {
  if (window.innerWidth <= 768) {
    if (sidebar.classList.contains("hidden")) {
      // 1. User is in chat -> Close chat, go directly to user list. No logout prompt.
      ...
      // 2. User is on user list -> Show disconnect prompt.
      if(confirm("Disconnect from Chit-Chat?")) {
